...
class Api extends REST_Controller {
    ...
    function historyDetail_post() {
        ...
        if (!$this->mdl_api->cektokenuser($id_user,$token)) {
            ...
            $temproduk['ongkir']=$pesananheader->ONGKIR_PESANAN;
    ...
    function subcategory_post() {
        ...
        if (!$this->mdl_api->cektokenuser($id_user,$token)) {
            ...
            $subcategory=$this->mdl_api->getsubcategory($id_category);
            $temp2=array(); $temp=array();
            foreach ($subcategory AS $s) {
                $temp['id']=$s->ID_SUB_KATEGORI;
                $temp['image']=$this->base_url_aplikasi.$s->GAMBAR_SUB_KATEGORI;
